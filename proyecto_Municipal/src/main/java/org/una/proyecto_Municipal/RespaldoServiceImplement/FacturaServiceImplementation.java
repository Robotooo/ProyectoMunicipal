...
public class FacturaServiceImplementation implements  IFacturaService{
    ...
    public Optional<List<FacturaDTO>> findAll() {
        List<FacturaDTO> facturaDTOList = MapperUtils.DtoListFromEntityList(facturaRepository.findAll(), FacturaDTO.class);
        return Optional.ofNullable(facturaDTOList);
    }

    @Override
    public Optional<List<FacturaDTO>> findByNombreAproximateIgnoreCase(String nombre) {
        List<Factura> facturaList = facturaRepository.findByNombreContainingIgnoreCase(nombre);
